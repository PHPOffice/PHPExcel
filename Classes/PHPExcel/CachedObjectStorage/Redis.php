...
/**
 * PHPExcel_CachedObjectStorage_Redis
 *
 ...
 */
class PHPExcel_CachedObjectStorage_Redis extends PHPExcel_CachedObjectStorage_CacheBase implements PHPExcel_CachedObjectStorage_ICache
{
    /**
    ...
    protected function storeData()
    {
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!$this->redis->setex($this->cachePrefix . $this->currentObjectID . '.cache', $this->cacheTime, $obj)) {
                $this->__destruct();
                throw new PHPExcel_Exception("Failed to store cell {$this->currentObjectID} in Redis");
            }
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            $success = $this->redis->exists($this->cachePrefix.$pCoord.'.cache');
            if ($success === false) {
                //    Entry no longer exists in Redis, so clear it from the cache array
                parent::deleteCacheData($pCoord);
                throw new PHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in Redis');
            }
            return true;
        }
        return false;
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($obj === false) {
                    ...
                    throw new PHPExcel_Exception("Cell entry {$cellID} no longer exists in Redis");
                }
                if (!$this->redis->setex($newCachePrefix . $cellID . '.cache', $this->cacheTime, $obj)) {
                    $this->__destruct();
                    throw new PHPExcel_Exception("Failed to store cell {$cellID} in Redis");
                }
            }
        }
