                                        ...
                                        if ($xmlDrawing->twoCellAnchor) {
                                            foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
                                                if ($twoCellAnchor->pic->blipFill) {
                                                    ...
                                                    if ($xfrm) {
                                                        $objDrawing->setWidth(\PhpOffice\PhpExcel\Shared_Drawing::EMUToPixels(self::getArrayItem($xfrm->ext->attributes(), "cx")));
                                                        $objDrawing->setHeight(\PhpOffice\PhpExcel\Shared_Drawing::EMUToPixels(self::getArrayItem($xfrm->ext->attributes(), "cy")));
                                                        $objDrawing->setRotation(\PhpOffice\PhpExcel\Shared_Drawing::angleToDegrees(self::getArrayItem($xfrm->attributes(), "rot")));
                                                    }
                                                    if ($outerShdw) {
                                                        $shadow = $objDrawing->getShadow();
                                                        $shadow->setVisible(true);
                ...
                switch ($contentType["ContentType"]) {
                    case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
                        if ($this->includeCharts) {
                            ...
                            $objChart = \PhpOffice\PhpExcel\Reader\Excel2007_Chart::readChart($chartElements, basename($chartEntryRef, '.xml'));
