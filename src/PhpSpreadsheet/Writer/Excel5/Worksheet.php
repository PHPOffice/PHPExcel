...
/**
 * \PhpOffice\PhpExcel\Writer\Excel5\Worksheet
 *
...
class Worksheet extends BIFFwriter
{
     ...
     * Formula parser
     *
     * @var \PhpOffice\PhpExcel\Writer\Excel5\Parser
     */
    private $parser;
     ...
     *
     * @var \PhpOffice\PhpExcel\Shared\Escher
     */
    private $escher;
     ...
     * @param \PhpOffice\PhpExcel\Worksheet $phpSheet
     */
...
//        $this->firstRowIndex = ($minR > 65535) ? 65535 : $minR;
        ...
        $this->firstColumnIndex = \PhpOffice\PhpExcel\Cell::columnIndexFromString($minC);
        $this->lastColumnIndex  = \PhpOffice\PhpExcel\Cell::columnIndexFromString($maxC);
...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
     ...
     *
     * @access public
     * @see \PhpOffice\PhpExcel\Writer\Excel5\Workbook::storeWorkbook()
     */
    public function close()
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \PhpOffice\PhpExcel\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParent()->getDefaultStyle()->getFont());
        }
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            $columnLetter = \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($i);
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
            ...
            if ($cVal instanceof \PhpOffice\PhpExcel\RichText) {
                ...
                foreach ($elements as $element) {
                    // FONT Index
                    if ($element instanceof \PhpOffice\PhpExcel\RichText\Run) {
                ...
                switch ($cell->getDatatype()) {
                    ...
                    case \PhpOffice\PhpExcel\Cell\DataType::TYPE_NULL:
                        if ($cVal === '' || $cVal === null) {
        ...
        foreach ($phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
            list($column, $row) = \PhpOffice\PhpExcel\Cell::coordinateFromString($coordinate);
            ...
            $this->writeUrl($row - 1, \PhpOffice\PhpExcel\Cell::columnIndexFromString($column) - 1, $url);
        }
        ...
        if (!empty($arrConditionalStyles)) {
            ...
            foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
                foreach ($conditionalStyles as $conditional) {
                    if ($conditional->getConditionType() == \PhpOffice\PhpExcel\Style\Conditional::CONDITION_EXPRESSION
                        || $conditional->getConditionType() == \PhpOffice\PhpExcel\Style\Conditional::CONDITION_CELLIS) {
    ...
    private function writeBIFF8CellRangeAddressFixed($range = 'A1')
    {
        ...
        $firstCellCoordinates = \PhpOffice\PhpExcel\Cell::coordinateFromString($firstCell); // e.g. array(0, 1)
        $lastCellCoordinates  = \PhpOffice\PhpExcel\Cell::coordinateFromString($lastCell);  // e.g. array(1, 6)

        return pack('vvvv', $firstCellCoordinates[1] - 1, $lastCellCoordinates[1] - 1, \PhpOffice\PhpExcel\Cell::columnIndexFromString($firstCellCoordinates[0]) - 1, \PhpOffice\PhpExcel\Cell::columnIndexFromString($lastCellCoordinates[0]) - 1);
    }
    ...
    private function writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
    {
        ...
        if (isset($calculatedValue)) {
            ...
            if (is_bool($calculatedValue)) {
                ...
                if (array_key_exists($calculatedValue, \PhpOffice\PhpExcel\Cell\DataType::getErrorCodes())) {
                    // Error value
    ...
    private function writeWindow2()
    {
        ...
        $fPaged         = 1;                     // 2
        $fPageBreakPreview = $this->phpSheet->getSheetView()->getView() === \PhpOffice\PhpExcel\Worksheet\SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
    ...
    private function writeSelection()
    {
        ...
        $selectedCells = \PhpOffice\PhpExcel\Cell::splitRange($this->phpSheet->getSelectedCells());
        ...
        list($colFirst, $rwFirst) = \PhpOffice\PhpExcel\Cell::coordinateFromString($first);
        $colFirst = \PhpOffice\PhpExcel\Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
        --$rwFirst; // base 0 row index

        list($colLast, $rwLast) = \PhpOffice\PhpExcel\Cell::coordinateFromString($last);
        $colLast = \PhpOffice\PhpExcel\Cell::columnIndexFromString($colLast) - 1; // base 0 column index
    ...
    private function writeMergedCells()
    {
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            $range = \PhpOffice\PhpExcel\Cell::splitRange($mergeCell);
            list($first, $last) = $range[0];
            list($firstColumn, $firstRow) = \PhpOffice\PhpExcel\Cell::coordinateFromString($first);
            list($lastColumn, $lastRow) = \PhpOffice\PhpExcel\Cell::coordinateFromString($last);

            $recordData .= pack('vvvv', $firstRow - 1, $lastRow - 1, \PhpOffice\PhpExcel\Cell::columnIndexFromString($firstColumn) - 1, \PhpOffice\PhpExcel\Cell::columnIndexFromString($lastColumn) - 1);
    ...
    private function writePanes()
    {
        ...
        if ($freezePane = $this->phpSheet->getFreezePane()) {
            list($column, $row) = \PhpOffice\PhpExcel\Cell::coordinateFromString($freezePane);
            $panes[0] = $row - 1;
            $panes[1] = \PhpOffice\PhpExcel\Cell::columnIndexFromString($column) - 1;
        } else {
            // thaw panes
            return;
        }
    ...
    private function writeSetup()
    {
        ...
        // Page orientation
        $fLandscape = ($this->phpSheet->getPageSetup()->getOrientation() == \PhpOffice\PhpExcel\Worksheet\PageSetup::ORIENTATION_LANDSCAPE) ?
            0x0 : 0x1;
    ...
    private function writeAutoFilterInfo()
    {
        ...
        $rangeBounds = \PhpOffice\PhpExcel\Cell::rangeBoundaries($this->phpSheet->getAutoFilter()->getRange());
    ...
    private function writeBreaks()
    {
        ...
        foreach ($this->phpSheet->getBreaks() as $cell => $breakType) {
            // Fetch coordinates
            $coordinates = \PhpOffice\PhpExcel\Cell::coordinateFromString($cell);

            // Decide what to do by the type of break
            switch ($breakType) {
                case \PhpOffice\PhpExcel\Worksheet::BREAK_COLUMN:
                    // Add to list of vertical breaks
                    $vbreaks[] = \PhpOffice\PhpExcel\Cell::columnIndexFromString($coordinates[0]) - 1;
                    break;
                case \PhpOffice\PhpExcel\Worksheet::BREAK_ROW:
                    // Add to list of horizontal breaks
                    $hbreaks[] = $coordinates[1];
                    break;
                case \PhpOffice\PhpExcel\Worksheet::BREAK_NONE:
                default:
                    // Nothing to do
                    break;
            }
    ...
    public function positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        if ($x1 >= \PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_start))) {
            $x1 = 0;
        }
        if ($y1 >= \PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_start + 1)) {
            $y1 = 0;
        }
        ...
        while ($width >= \PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_end))) {
            $width -= \PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_end));
            ++$col_end;
        }

        // Subtract the underlying cell heights to find the end cell of the image
        while ($height >= \PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_end + 1)) {
            $height -= \PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_end + 1);
            ++$row_end;
        }
        ...
        //
        if (\PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_start)) == 0) {
            return;
        }
        if (\PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_end))   == 0) {
            return;
        }
        if (\PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_start + 1) == 0) {
            return;
        }
        if (\PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_end + 1)   == 0) {
            return;
        }

        // Convert the pixel values to the percentage value expected by Excel
        $x1 = $x1     / \PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_start))   * 1024;
        $y1 = $y1     / \PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_start + 1)   *  256;
        $x2 = $width  / \PhpOffice\PhpExcel\Shared\Excel5::sizeCol($this->phpSheet, \PhpOffice\PhpExcel\Cell::stringFromColumnIndex($col_end))     * 1024; // Distance to right side of object
        $y2 = $height / \PhpOffice\PhpExcel\Shared\Excel5::sizeRow($this->phpSheet, $row_end + 1)     *  256; // Distance to bottom of object
    ...
    public function processBitmap($bitmap)
    {
        ...
        if (!$bmp_fd) {
            throw new \PhpOffice\PhpExcel\Writer\Exception("Couldn't import $bitmap");
        }
        ...
        if (strlen($data) <= 0x36) {
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap doesn't contain enough data.\n");
        }

        // The first 2 bytes are used to identify the bitmap.
        $identity = unpack("A2ident", $data);
        if ($identity['ident'] != "BM") {
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap doesn't appear to be a valid bitmap image.\n");
        }
        ...
        $data   = substr($data, 8);
        if ($width > 0xFFFF) {
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap: largest image width supported is 65k.\n");
        }
        if ($height > 0xFFFF) {
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap: largest image height supported is 65k.\n");
        }
        ...
        if ($planes_and_bitcount[2] != 24) { // Bitcount
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap isn't a 24bit true color bitmap.\n");
        }
        if ($planes_and_bitcount[1] != 1) {
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap: only 1 plane supported in bitmap image.\n");
        }
        ...
        if ($compression['comp'] != 0) {
            throw new \PhpOffice\PhpExcel\Writer\Exception("$bitmap: compression not supported in bitmap image.\n");
        }
     ...
     * Get Escher object
     *
     * @return \PhpOffice\PhpExcel\Shared\Escher
     */
     ...
     * Set Escher object
     *
     * @param \PhpOffice\PhpExcel\Shared\Escher $pValue
     */
    public function setEscher(\PhpOffice\PhpExcel\Shared\Escher $pValue = null)
    {
        $this->escher = $pValue;
    }
    ...
    private function writeMsoDrawing()
    {
        ...
        if (isset($this->escher)) {
            $writer = new \PhpOffice\PhpExcel\Writer\Excel5\Escher($this->escher);
            $data = $writer->close();
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                switch ($type) {
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_NONE:
                        $type = 0x00;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_WHOLE:
                        $type = 0x01;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_DECIMAL:
                        $type = 0x02;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_LIST:
                        $type = 0x03;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_DATE:
                        $type = 0x04;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_TIME:
                        $type = 0x05;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_TEXTLENGTH:
                        $type = 0x06;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::TYPE_CUSTOM:
                        $type = 0x07;
                        break;
                }
                $options |= $type << 0;
                ...
                switch ($errorStyle) {
                    case \PhpOffice\PhpExcel\Cell\DataValidation::STYLE_STOP:
                        $errorStyle = 0x00;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::STYLE_WARNING:
                        $errorStyle = 0x01;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::STYLE_INFORMATION:
                        $errorStyle = 0x02;
                        break;
                }
                ...
                switch ($operator) {
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_BETWEEN:
                        $operator = 0x00;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_NOTBETWEEN:
                        $operator = 0x01;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_EQUAL:
                        $operator = 0x02;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_NOTEQUAL:
                        $operator = 0x03;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_GREATERTHAN:
                        $operator = 0x04;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_LESSTHAN:
                        $operator = 0x05;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_GREATERTHANOREQUAL:
                        $operator = 0x06;
                        break;
                    case \PhpOffice\PhpExcel\Cell\DataValidation::OPERATOR_LESSTHANOREQUAL:
                        $operator = 0x07;
                        break;
                }
                $options |= $operator << 20;
                ...
                try {
                    ...
                    if ($formula2 === '') {
                        throw new \PhpOffice\PhpExcel\Writer\Exception('No formula2');
                    }
    ...
    private function writePageLayoutView()
    {
        ...
        // The options flags that comprise $grbit
        if ($this->phpSheet->getSheetView()->getView() == \PhpOffice\PhpExcel\Worksheet\SheetView::SHEETVIEW_PAGE_LAYOUT) {
            $fPageLayoutView   = 1;
        } else {
            $fPageLayoutView   = 0;
        }
    ...
    /**
     * Write CFRule Record
     * @param \PhpOffice\PhpExcel\Style\Conditional $conditional
     */
    private function writeCFRule(\PhpOffice\PhpExcel\Style\Conditional $conditional)
    {
        ...
        if ($conditional->getConditionType() == \PhpOffice\PhpExcel\Style\Conditional::CONDITION_EXPRESSION) {
            ...
            switch ($conditional->getOperatorType()) {
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_NONE:
                    $operatorType = 0x00;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_EQUAL:
                    $operatorType = 0x03;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_GREATERTHAN:
                    $operatorType = 0x05;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_GREATERTHANOREQUAL:
                    $operatorType = 0x07;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_LESSTHAN:
                    $operatorType = 0x06;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_LESSTHANOREQUAL:
                    $operatorType = 0x08;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_NOTEQUAL:
                    $operatorType = 0x04;
                    break;
                case \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_BETWEEN:
                    $operatorType = 0x01;
                    break;
        ...
        if ($numConditions == 1) {
            ...
            $operand2 = null;
        } elseif ($numConditions == 2 && ($conditional->getOperatorType() == \PhpOffice\PhpExcel\Style\Conditional::OPERATOR_BETWEEN)) {
            $szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
        ...
        // Border
        $bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB() == \PhpOffice\PhpExcel\Style\Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getLeft()->getBorderStyle() == \PhpOffice\PhpExcel\Style\Border::BORDER_NONE ? 1 : 0);
        $bBorderRight = ($conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB() == \PhpOffice\PhpExcel\Style\Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getRight()->getBorderStyle() == \PhpOffice\PhpExcel\Style\Border::BORDER_NONE ? 1 : 0);
        $bBorderTop = ($conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB() == \PhpOffice\PhpExcel\Style\Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getTop()->getBorderStyle() == \PhpOffice\PhpExcel\Style\Border::BORDER_NONE ? 1 : 0);
        $bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB() == \PhpOffice\PhpExcel\Style\Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getBottom()->getBorderStyle() == \PhpOffice\PhpExcel\Style\Border::BORDER_NONE ? 1 : 0);
        ...
        if ($bFormatFont == 1) {
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case \PhpOffice\PhpExcel\Style\Font::UNDERLINE_NONE:
                    $dataBlockFont .= pack('C', 0x00);
                    $fontUnderline = 0;
                    break;
                case \PhpOffice\PhpExcel\Style\Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= pack('C', 0x02);
                    $fontUnderline = 0;
                    break;
                case \PhpOffice\PhpExcel\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= pack('C', 0x22);
                    $fontUnderline = 0;
                    break;
                case \PhpOffice\PhpExcel\Style\Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= pack('C', 0x01);
                    $fontUnderline = 0;
                    break;
                case \PhpOffice\PhpExcel\Style\Font::UNDERLINE_SINGLEACCOUNTING:
        ...
        if ($bFormatAlign == 1) {
            ...
            switch ($conditional->getStyle()->getAlignment()->getHorizontal()) {
                case \PhpOffice\PhpExcel\Style\Alignment::HORIZONTAL_GENERAL:
                    $blockAlign = 0;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::HORIZONTAL_LEFT:
                    $blockAlign = 1;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::HORIZONTAL_RIGHT:
                    $blockAlign = 3;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::HORIZONTAL_CENTER:
                    $blockAlign = 2;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::HORIZONTAL_CENTER_CONTINUOUS:
                    $blockAlign = 6;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::HORIZONTAL_JUSTIFY:
                    $blockAlign = 5;
                    break;
            }
            ...
            switch ($conditional->getStyle()->getAlignment()->getVertical()) {
                case \PhpOffice\PhpExcel\Style\Alignment::VERTICAL_BOTTOM:
                    $blockAlign = 2 << 4;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::VERTICAL_TOP:
                    $blockAlign = 0 << 4;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::VERTICAL_CENTER:
                    $blockAlign = 1 << 4;
                    break;
                case \PhpOffice\PhpExcel\Style\Alignment::VERTICAL_JUSTIFY:
                    $blockAlign = 3 << 4;
                    break;
            }
            $blockAlign |= 0 << 7;
        ...
        if ($bFormatBorder == 1) {
            ...
            switch ($conditional->getStyle()->getBorders()->getLeft()->getBorderStyle()) {
                case \PhpOffice\PhpExcel\Style\Border::BORDER_NONE:
                    $blockLineStyle |= 0x00;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_THIN:
                    $blockLineStyle |= 0x01;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_MEDIUM:
                    $blockLineStyle |= 0x02;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_DASHED:
                    $blockLineStyle |= 0x03;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_DOTTED:
                    $blockLineStyle |= 0x04;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_THICK:
                    $blockLineStyle |= 0x05;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_DOUBLE:
                    $blockLineStyle |= 0x06;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_HAIR:
                    $blockLineStyle |= 0x07;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_MEDIUMDASHED:
                    $blockLineStyle |= 0x08;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_DASHDOT:
                    $blockLineStyle |= 0x09;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_MEDIUMDASHDOT:
                    $blockLineStyle |= 0x0A;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_DASHDOTDOT:
                    $blockLineStyle |= 0x0B;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_MEDIUMDASHDOTDOT:
                    $blockLineStyle |= 0x0C;
                    break;
                case \PhpOffice\PhpExcel\Style\Border::BORDER_SLANTDASHDOT:
                    $blockLineStyle |= 0x0D;
                    break;
            }
        ...
        if ($bFormatFill == 1) {
            ...
            switch ($conditional->getStyle()->getFill()->getFillType()) {
                case \PhpOffice\PhpExcel\Style\Fill::FILL_NONE:
                    $blockFillPatternStyle = 0x00;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_SOLID:
                    $blockFillPatternStyle = 0x01;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_MEDIUMGRAY:
                    $blockFillPatternStyle = 0x02;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKGRAY:
                    $blockFillPatternStyle = 0x03;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTGRAY:
                    $blockFillPatternStyle = 0x04;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKHORIZONTAL:
                    $blockFillPatternStyle = 0x05;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKVERTICAL:
                    $blockFillPatternStyle = 0x06;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKDOWN:
                    $blockFillPatternStyle = 0x07;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKUP:
                    $blockFillPatternStyle = 0x08;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKGRID:
                    $blockFillPatternStyle = 0x09;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_DARKTRELLIS:
                    $blockFillPatternStyle = 0x0A;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTHORIZONTAL:
                    $blockFillPatternStyle = 0x0B;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTVERTICAL:
                    $blockFillPatternStyle = 0x0C;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTDOWN:
                    $blockFillPatternStyle = 0x0D;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTUP:
                    $blockFillPatternStyle = 0x0E;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTGRID:
                    $blockFillPatternStyle = 0x0F;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_LIGHTTRELLIS:
                    $blockFillPatternStyle = 0x10;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_GRAY125:
                    $blockFillPatternStyle = 0x11;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_PATTERN_GRAY0625:
                    $blockFillPatternStyle = 0x12;
                    break;
                case \PhpOffice\PhpExcel\Style\Fill::FILL_GRADIENT_LINEAR:
                    $blockFillPatternStyle = 0x00;
                    break;    // does not exist in BIFF8
                case \PhpOffice\PhpExcel\Style\Fill::FILL_GRADIENT_PATH:
                    $blockFillPatternStyle = 0x00;
                    break;    // does not exist in BIFF8
                default:
                    $blockFillPatternStyle = 0x00;
        ...
        if ($bFormatProt == 1) {
            ...
            if ($conditional->getStyle()->getProtection()->getLocked() == \PhpOffice\PhpExcel\Style\Protection::PROTECTION_PROTECTED) {
                $dataBlockProtection = 1;
            }
            if ($conditional->getStyle()->getProtection()->getHidden() == \PhpOffice\PhpExcel\Style\Protection::PROTECTION_PROTECTED) {
        ...
        foreach ($this->phpSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            foreach ($conditionalStyles as $conditional) {
                if ($conditional->getConditionType() == \PhpOffice\PhpExcel\Style\Conditional::CONDITION_EXPRESSION
                    ...
                    // Cells
                    $arrCoord = \PhpOffice\PhpExcel\Cell::coordinateFromString($cellCoordinate);
                    if (!is_numeric($arrCoord[0])) {
                        $arrCoord[0] = \PhpOffice\PhpExcel\Cell::columnIndexFromString($arrCoord[0]);
                    }
